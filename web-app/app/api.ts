...
async function odooExecuteKw(
    ...
    const { ODOO_DB: db } = process.env;
    const uid = parseInt(credentials.cred_odoo_uid, 10);
    const apiKey = credentials.cred_odoo_api_key;
...
export async function odooCallMethod(model: string, ids: number[], method: string) {
...
export async function callBit2win(
    ...
    const op = async (token: string) => {
        const { B2W_URL: b2wUrl = '' } = process.env;
        let parsedUrl = url;
