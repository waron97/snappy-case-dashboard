...
import { odooNameGet, odooRead, odooWrite } from '../../../api';
...
export default function FullRecordView() {
    ...
    const BROKEN_FIELDS: Record<string, Set<string>> = {
        ...
        'service.point': new Set(['placeholder_service_id']),
    };
    ...
    const fieldNames = fieldDefs
        ? Object.entries(fieldDefs)
              .filter(
                  ([name, def]) =>
                      def.store &&
                      !SKIP_TYPES.has(def.type) &&
                      !BROKEN_FIELDS[String(model)]?.has(name)
              )
              .map(([name]) => name)
        : [];
    ...
    function renderEditInput(fieldName: string, fieldDef: OdooFieldDefinition, rawValue: unknown) {
        ...
        function onChange(newVal: unknown) {
            if (
                newVal === rawValue ||
                (newVal === '' &&
                    (rawValue === false || rawValue === null || rawValue === undefined))
            ) {
                setEdits((prev) => {
                    const next = { ...prev };
        ...
        if (fieldDef.type === 'selection') {
            const data = (fieldDef.selection ?? []).map(([v, l]) => ({
                value: String(v),
                label: l,
            }));
            return (
    ...
    const fmt = (v: unknown) =>
        v && typeof v === 'string' ? dayjs(v).format('D/M/YY HH:mm') : '—';
    ...
    return (
        <Container size="xl" py="xl">
            <Group mb="md" align="center" justify="space-between">
                ...
                <Anchor
                    href={`${process.env.ODOO_URL}/web#id=${id}&model=${model}&view_type=form`}
                    target="_blank"
                >
                    ...
                    <UiCard title="Record metadata">
                        <Stack gap="sm">
                            ...
                            <MetaRow
                                ...
                                value={userName(recordData?.write_uid)}
                            />
                        </Stack>
                    </UiCard>
                </Grid.Col>
