                            ...
                            <>
                                ...
                                <Grid>
                                    <Grid.Col span={6}>
                                        <Stack gap="sm">
                                            {data.toponym_id &&
                                                item('Toponym', rel(data.toponym_id) as string)}
                                            {streetLine && item('Street', streetLine)}
                                            {data.street2 && item('Street 2', data.street2)}
                                            {data.apartment_number &&
                                                item('Apartment', data.apartment_number)}
                                    ...
                                    <Grid.Col span={6}>
                                        <Stack gap="sm">
                                            ...
                                            {data.state_id &&
                                                item('Province', rel(data.state_id) as string)}
                                            {data.country_id &&
                                                item('Country', rel(data.country_id) as string)}
                                            {data.parent_id &&
                                                item('Company', rel(data.parent_id) as string)}
                                        </Stack>
                                    </Grid.Col>
                                </Grid>
                            </>
                                ...
                                <Grid>
                                    <Grid.Col span={6}>
                                        <Stack gap="sm">
                                            ...
                                            {data.business_type &&
                                                item('Business type', data.business_type)}
                                        </Stack>
                                    </Grid.Col>
                                    <Grid.Col span={6}>
                                        <Stack gap="sm">
                                            {data.sdi_code && item('SDI code', data.sdi_code)}
                                        </Stack>
                                    </Grid.Col>
                                </Grid>
                            </>
                                    ...
                                    <Grid.Col span={6}>
                                        <Stack gap="sm">
                                            {data.birth_date &&
                                                item(
                                                    'Birth date',
                                                    dayjs(data.birth_date).format('D/M/YYYY')
                                                )}
                                        </Stack>
                                    </Grid.Col>
                                    <Grid.Col span={6}>
                        ...
                        {hasStatus && (
                            <>
                                ...
                                <Stack gap="sm">
                                    ...
                                    {data.total_client_status &&
                                        item('Client status', data.total_client_status)}
                                    {data.credit_status &&
                                        item('Credit status', rel(data.credit_status) as string)}
                                </Stack>
                            </>
                        )}
                    </Stack>
