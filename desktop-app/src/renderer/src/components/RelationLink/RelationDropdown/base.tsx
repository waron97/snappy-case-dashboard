...
import { Anchor, Button, Grid, Group, Stack, Tabs, Text, Title } from '@mantine/core';
...
export default function RelationDropdownBase(props: Props) {
    ...
    function renderContent() {
        ...
        if (model === 'res.partner') {
            return <ResPartner id={pgId} />;
        }

        if (model === 'service.point') {
            return <ServicePoint id={pgId} />;
        }
        ...
        <Stack gap="sm">
            <Group justify="space-between">
                ...
                <Group gap="sm">
                    ...
                    <Anchor
                        href={`${settings?.odooUrl}/web#id=${pgId}&model=${model}&view_type=form`}
                        target="_blank"
                    >
                        <Button bg="#714B67">ODOO</Button>
                    </Anchor>
                    <Link to={`/full-field-config/${model}/${pgId}`}>
                        <Button>
                            <IconCode />
                        </Button>
                    </Link>
