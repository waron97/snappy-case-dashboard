  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      odooRead(
        'rip.request.log',
        [id!],
        ['endpoint', 'method', 'status', 'args', 'content', 'response_content', 'create_date']
      ),
    enabled: id !== null
  })
          ...
          <Tabs.Panel value="general" pt="md">
            <Stack gap="xs">
              ...
              <Group gap="xs" align="center">
                ...
                <Badge color={log.status >= 200 && log.status < 300 ? 'green' : 'red'}>
                  {log.status}
                </Badge>
              </Group>
              <Text>
                <strong>Date:</strong> {dayjs(log.create_date).format('D/M/YY HH:mm')}
              </Text>
